...
const AnalyticsBarChart = dynamic(() => import('@/components/analytics/AnalyticsCharts').then(m => m.AnalyticsBarChart), {
  ssr: false,
...
const AnalyticsLineChart = dynamic(() => import('@/components/analytics/AnalyticsCharts').then(m => m.AnalyticsLineChart), {
  ssr: false,
  loading: () => <div className="h-[180px] bg-gray-50/50 rounded-xl animate-pulse flex items-center justify-center text-xs text-gray-400">Loading chart…</div>
});
...
function BlockRenderer({ block }: { block: AnalyticsBlock }) {
  ...
  if (block.type === 'bar_chart') {
    return (
      <div className="my-3 bg-white border border-gray-100 rounded-xl p-3">
        ...
        <AnalyticsBarChart data={block.data} unit={block.unit} GREEN={GREEN} />
      </div>
    );
  }
  ...
  if (block.type === 'line_chart') {
    return (
      <div className="my-3 bg-white border border-gray-100 rounded-xl p-3">
        {block.title && <p className="text-xs font-semibold text-gray-700 mb-2">{block.title}</p>}
        <AnalyticsLineChart data={block.data} unit={block.unit} GREEN={GREEN} />
      </div>
    );
  }
