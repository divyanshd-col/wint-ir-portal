...
// ── KB citation cleanup ───────────────────────────────────────────────────────
// Strips raw Google Drive file IDs (25+ char base62 strings) that may appear
// before '>' in the citation, and formats multiple citations with '·' separator.
function cleanKbCitation(raw: string): string {
  // Remove "DRIVE_ID > " prefix patterns
  let s = raw.replace(/[A-Za-z0-9_-]{25,}\s*>\s*/g, '');
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_300px_300px] gap-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
            <div className="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center gap-3">
              ...
              <div className="flex rounded-lg overflow-hidden border border-slate-200 text-[10px] font-semibold">
                {(['merged', 'calls', 'chat'] as const).map(v => (
                  <button
                    key={v}
              ...
              {timelineView === 'calls' && (
                callRecordings.length > 0 ? (
                  <div className="space-y-3">
                    {callRecordings.map((rec: any, i: number) => (
                      <CallTranscriptCard
                        key={rec.id}
                        rec={rec}
                        index={i}
                ...
                ) : <p className="text-sm text-slate-400 text-center py-8">No call recordings for this chat.</p>
              )}
              {timelineView === 'chat' && chatMessages.map((item: any, i: number) => (
                <TimelineItem key={i} item={item} />
              ))}
              {(timelineView === 'merged' ? mergedItems : timelineView === 'chat' ? chatMessages : []).length === 0 && timelineView !== 'calls' && (
          ...
          <ParamPanel
            ...
            kbCitation={result.chatKbCitation ? cleanKbCitation(result.chatKbCitation) : null}
          />
          ...
          <ParamPanel
            ...
            kbCitation={result.callKbCitation ? cleanKbCitation(result.callKbCitation) : null}
          />
        </div>
