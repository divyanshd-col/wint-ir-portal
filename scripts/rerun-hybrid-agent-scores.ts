...
async function main() {
  ...
  console.log('--- STARTING HYBRID/AGENT CHATS RE-RUN PROCESS ---');
  ...
  const convs = rawConvs.filter((conv: any) => {
    ...
    // For hybrid/agent, there MUST be at least one human message (or trust the DB)
    return hasHuman || conv.conversation_type === 'agent' || conv.conversation_type === 'hybrid';
  });

  const total = convs.length;
  console.log(`- Found ${total} Hybrid/Agent (Type 2/3) chats to re-run.\n`);
